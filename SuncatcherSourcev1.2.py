...
long_home =  YY.YYYY        # Car's longitude when parked at user's home
    ...
    if at_home == "At Home" and tpi == "Plugged In" and rate == "Off-Peak" and charge_level <= max_day_battery_level:
        ...
        if current_meter_reading >= pull_threshold:
            if currently_charging == "Charging":
                ...
                solar()
                write_log()
        elif current_meter_reading < pull_threshold and charge_level <= max_day_battery_level and new_amps > 0:
            if currently_charging == "Charging":
            ...
            print("\nResult:  Not pulling from grid or givig back to grid.  No changes will be made.")
            print("Current car charging amps:  %s\n\n" % current_amps)
            new_amps = current_amps
            solar()
            write_log()
    else:
        if currently_charging == "Charging" and at_home == "Not At Home":
            print("\nResult:  Car is charging away from Home.  No changes will be made.")
            new_amps = current_amps
            solar()
            write_log()
        else:
            print("\nResult:  Car not able to be charged. (In peak electric rate, min night charge reached, car not plugged in, etc.)  No action taken.")
            new_amps = 0
            solar()
            write_log()
    sys.stdout.flush()
